...
void GLSLProgram::enableVertexAttribArray(const GLint handle)
{
  if(handle >= 0) {
...
void GLSLProgram::disableVertexAttribArray(const GLint handle)
{
  if(handle >= 0) {
...
void GLSLProgram::vertexAttribPointer(const GLint handle, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const GLvoid *pointer)
{
  if(handle >= 0) {
...
int GLSLProgram::checkForOpenGLError(const char * file, int line) {
    //
