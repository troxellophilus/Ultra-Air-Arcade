 ...
 */
int DrawText::initResources(int w, int h) {
   width = w;
   height = h;
...
void DrawText::renderText(const char *text, int font, float x, float y, float sx, float sy) {
   ...
   for (p = text; *p; p++) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, slot->bitmap.width, slot->bitmap.rows, 0, GL_RED, GL_UNSIGNED_BYTE, slot->bitmap.buffer);
